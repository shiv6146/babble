...
void *execute_command(void *ignored) {
    ...
    while(1) {
        ...
        int k;
        for (k = 0; k < BABBLE_PUB_BUF_SIZE; k++) {
            if (pub_cmd_buf[k] != NULL && pub_cmd_buf[k]->processed == 0) {
                command_t *pub_cmd = pub_cmd_buf[k];
                client_bundle_t *tmp = registration_lookup(pub_cmd->key);
                if (tmp->disconnected == 0) {
                    answer_t *pub_ans = NULL;
                    if(process_command(pub_cmd, &pub_ans) == -1){
...
void *process_client(void *client_sock) {
    ...
    while((recv_size=network_recv(newsockfd, (void**) &recv_buff)) > 0){
        ...
        if(parse_command(recv_buff, cmd) == -1){
            ...
            if (cmd->cid == PUBLISH) {
                pub_cmd_buf[pub_in] = cmd;
                pub_in = (pub_in + 1) % BABBLE_PUB_BUF_SIZE;
            } else if (cmd->cid > PUBLISH && cmd->cid < UNREGISTER) {
                cmd_buf[in] = cmd;
