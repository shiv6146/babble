...
int run_login_command(command_t *cmd, answer_t **answer)
{
    ...
    printf("Client key: [%ld] Client name: [%s]\n", client_data->key, client_data->client_name);
...
int run_publish_command(command_t *cmd, answer_t **answer)
{
    ...
    if (client->nb_followers >= BABBLE_CELEBRITY_THRESHOLD) {
        printf("[%s] is a celebrity\n", client->client_name);
        client->is_celebrity = 1;
        celeb_publication_t *new_pub = {cmd->msg, NULL};
        
        celeb_publication_t *tmp = client->publication_head;
        if (tmp == NULL) {
            client->publication_head = new_pub;
        } else {
            new_pub->next = tmp;
            client->publication_head = new_pub;
        }
